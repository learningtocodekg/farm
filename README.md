...
### Dual-Agent Photo Analysis
Upload 1–20 farm photos and two Gemini agents process them **in parallel**:

```
Photos (1–20, in groups of 4 per point: N/S/E/W)
      │
      ▼
┌──────────────────────────────────────────────┐
│              FastAPI Backend                 │
│                                              │
│  Batch 1 (up to 8 photos) → Gemini Agent 1  │
│                                         ├───┴─→ Aggregated Results
│  Batch 2 (up to 8 photos) → Gemini Agent 2  │
│                                              │
└──────────────────────────────────────────────┘
```

Each group of 4 photos = one **Point** on the farm (North, South, East, West views). Gemini returns structured JSON identifying anomaly type, direction, and severity per point. Results are overlaid on the 3D viewer as problem markers.
